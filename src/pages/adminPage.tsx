...
const AdminPage = () => {
  ...
  const [builds, setBuilds] = useState<buildsDataType>({'web':[],  'leetcode':[], 'other': [] });
  ...
  useEffect(() => {
    ...
    if (allCurrentProjects !== null) {
      try {
        const parsedProjects = JSON.parse(allCurrentProjects) as projectDataType[];
  ...
  const addItem = async(type:string, newData: projectDataType[] | buildsDataType) => {
    let dataToAdd = null;

    if(type === 'project'){dataToAdd = {projects: newData}}
    else if (type === 'build'){dataToAdd = {buildData: newData }}
    ...
    try{
      const response = await fetch(`https://getpantry.cloud/apiv1/pantry/${pantryId}/basket/${basketName}`, options)
    }catch(error){
      console.error("Couldn't add item to the API: " + error)
    }
  }

  const closeSidePanel = (type?:"project" | "build" | "card" | null, newData?: projectDataType | buildsDataType) => {
    ...
    <div className="w-full min-h-screen font-mono flex flex-col bg-slate-200 text-black dark:bg-gray-900  dark:text-white p-3">
      ...
      <div className='grid grid-cols-4 gap-3'>
        {projects.map((project, index) => (
          <div key={index} className="mb-4 p-4 shadow rounded">
            ...
            <a href={project.githubLink} className="text-blue-600">{project.githubLink}</a>
      ...
      <div className='grid grid-cols-4 gap-3'>
        {builds.web.map((build, index) => {
          ...
          return <div key={index} className="mb-4 p-4 shadow rounded">
            <h2 className="text-2xl font-bold">{build.type}</h2>
            <a target='_blank' href={preType}> {build.id}</a>
          </div>
        })}
